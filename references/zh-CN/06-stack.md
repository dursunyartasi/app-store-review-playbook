...
- `docker image prune -af` —— 任何时候都安全。
- `docker builder prune -af` —— **绝不要在部署前做。** 它会删掉构建缓存，于是
  Dockerfile 里的 `apt-get` 层重新执行并重新下载软件包；一次网络抖动就足以让构建
  失败。不在部署路径上时，它是你手里最有效的手段。

镜像大多是被引用着的，清理它们释放不了多少。更多 Coolify 故障模式——包括三种产生
同一个错误的不同原因——见 [Coolify 运维手册](https://github.com/dursunyartasi/coolify-operations-playbook)。**千万别碰 volume——那是你的应用数据。** 有一次事故里，这一步把磁盘从 92% 降到 83%，释放了 7.6 GB；重试之后部署就成功了。
